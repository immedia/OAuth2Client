...
NSString *const NXOAuth2AccountStoreDidFailToRequestAccessNotification =
...
NSString *const NXOAuth2AccountStoreNewAccountUserInfoKey =
    @"NXOAuth2AccountStoreNewAccountUserInfoKey";

#pragma mark Configuration

NSString *const kNXOAuth2AccountStoreConfigurationClientID =
    @"kNXOAuth2AccountStoreConfigurationClientID";
NSString *const kNXOAuth2AccountStoreConfigurationSecret =
    @"kNXOAuth2AccountStoreConfigurationSecret";
NSString *const kNXOAuth2AccountStoreConfigurationAuthorizeURL =
    @"kNXOAuth2AccountStoreConfigurationAuthorizeURL";
NSString *const kNXOAuth2AccountStoreConfigurationTokenURL =
    @"kNXOAuth2AccountStoreConfigurationTokenURL";
NSString *const kNXOAuth2AccountStoreConfigurationRedirectURL =
    @"kNXOAuth2AccountStoreConfigurationRedirectURL";
NSString *const kNXOAuth2AccountStoreConfigurationScope =
    @"kNXOAuth2AccountStoreConfigurationScope";
NSString *const kNXOAuth2AccountStoreConfigurationTokenType =
    @"kNXOAuth2AccountStoreConfigurationTokenType";
NSString *const kNXOAuth2AccountStoreConfigurationTokenRequestHTTPMethod =
    @"kNXOAuth2AccountStoreConfigurationTokenRequestHTTPMethod";
NSString *const kNXOAuth2AccountStoreConfigurationKeyChainGroup =
    @"kNXOAuth2AccountStoreConfigurationKeyChainGroup";
NSString *const kNXOAuth2AccountStoreConfigurationKeyChainAccessGroup =
    @"kNXOAuth2AccountStoreConfigurationKeyChainAccessGroup";
NSString *const kNXOAuth2AccountStoreConfigurationAdditionalAuthenticationParameters =
    @"kNXOAuth2AccountStoreConfigurationAdditionalAuthenticationParameters";
NSString *const kNXOAuth2AccountStoreConfigurationCustomHeaderFields =
    @"kNXOAuth2AccountStoreConfigurationCustomHeaderFields";
...
- (void)storeAccountsInDefaultKeychain:(NSDictionary *)accounts
                       withAccessGroup:(NSString *)keyChainAccessGroup;
...
{
    if (accountsDict == nil) {
        accountsDict = [NSMutableDictionary
...
- (void)requestAccessToAccountWithType:(NSString *)accountType
   withPreparedAuthorizationURLHandler:
       (NXOAuth2PreparedAuthorizationURLHandler)aPreparedAuthorizationURLHandler;
{
...
- (void)requestAccessToAccountWithExternalProviderWithType:(NSString *)accountType
                                                 withToken:(NSString *)token;
{
    NXOAuth2Client *client = [self pendingOAuthClientForAccountType:accountType];
    [client authenticateFromExternalProviderWithToken:token];
}

- (void)requestAccessToAccountWithExternalProviderWithType:(NSString *)accountType
                                                 withToken:(NSString *)token andEmail:(NSString*)email;
{
    NXOAuth2Client *client = [self pendingOAuthClientForAccountType:accountType];
    [client authenticateFromExternalProviderWithToken:token andEmail:email];
}

- (void)requestAccessToAccountWithType:(NSString *)accountType
                         assertionType:(NSURL *)assertionType
                             assertion:(NSString *)assertion;
{
    NXOAuth2Client *client = [self pendingOAuthClientForAccountType:accountType];
...
- (void)removeAccount:(NXOAuth2Account *)account;
{
    if (account) {
        @synchronized(self.accountsDict) {
            ...
            [self storeAccountsInDefaultKeychain:self.accountsDict
                                 withAccessGroup:self.keychainAccessGroup];
        }
        [[NSNotificationCenter defaultCenter]
     ...
     forAccountType:(NSString *)anAccountType;
{
    NSMutableDictionary *config = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:aClientID, kNXOAuth2AccountStoreConfigurationClientID, aSecret,
                                     kNXOAuth2AccountStoreConfigurationSecret, anAuthorizationURL,
                                     kNXOAuth2AccountStoreConfigurationAuthorizeURL, aTokenURL,
                                     kNXOAuth2AccountStoreConfigurationTokenURL, aRedirectURL,
                                     kNXOAuth2AccountStoreConfigurationRedirectURL, nil];
     ...
     forAccountType:(NSString *)anAccountType;
{
    ...
    NSMutableDictionary *config = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:aClientID, kNXOAuth2AccountStoreConfigurationClientID, aSecret,
                                     kNXOAuth2AccountStoreConfigurationSecret, theScope,
                                     kNXOAuth2AccountStoreConfigurationScope, anAuthorizationURL,
                                     kNXOAuth2AccountStoreConfigurationAuthorizeURL, aTokenURL,
                                     kNXOAuth2AccountStoreConfigurationTokenURL, aKeyChainGroup,
                                     kNXOAuth2AccountStoreConfigurationKeyChainGroup, aRedirectURL,
                                     kNXOAuth2AccountStoreConfigurationRedirectURL, nil];
     ...
     forAccountType:(NSString *)anAccountType;
{
    ...
    NSMutableDictionary *config = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:aClientID, kNXOAuth2AccountStoreConfigurationClientID, aSecret,
                                     ...
                                     kNXOAuth2AccountStoreConfigurationScope, anAuthorizationURL,
                                     kNXOAuth2AccountStoreConfigurationAuthorizeURL, aTokenURL,
                                     kNXOAuth2AccountStoreConfigurationTokenURL, aTokenType,
                                     kNXOAuth2AccountStoreConfigurationTokenType, aKeyChainGroup,
                                     kNXOAuth2AccountStoreConfigurationKeyChainGroup, aRedirectURL,
                                     kNXOAuth2AccountStoreConfigurationRedirectURL, nil];
...
{
    NSAssert1([configuration objectForKey:kNXOAuth2AccountStoreConfigurationClientID],
              @"Missing OAuth2 client ID for account type '%@'.", accountType);
    NSAssert1([configuration objectForKey:kNXOAuth2AccountStoreConfigurationSecret],
              @"Missing OAuth2 client secret for account type '%@'.", accountType);
    NSAssert1([configuration objectForKey:kNXOAuth2AccountStoreConfigurationAuthorizeURL],
              @"Missing OAuth2 authorize URL for account type '%@'.", accountType);
    NSAssert1([configuration objectForKey:kNXOAuth2AccountStoreConfigurationTokenURL],
              @"Missing OAuth2 token URL for account type '%@'.", accountType);
...
- (NXOAuth2TrustedCertificatesHandler)trustedCertificatesHandlerForAccountType:
    (NSString *)accountType;
{
    NXOAuth2TrustedCertificatesHandler handler =
        [self.trustedCertificatesHandler objectForKey:accountType];
    NSAssert(handler, @"You need to provied a NXOAuth2TrustedCertificatesHandler for account type "
             @"'%@' because you are using 'NXOAuth2TrustModeSpecificCertificate' as "
             @"trust mode for that account type.",
             accountType);
    return handler;
}
...
- (NXOAuth2Client *)pendingOAuthClientForAccountType:(NSString *)accountType;
{
    ...
    @synchronized(self.pendingOAuthClients) {
        ...
        if (!client) {
            ...
            NSString *tokenRequestHTTPMethod = [configuration
                objectForKey:kNXOAuth2AccountStoreConfigurationTokenRequestHTTPMethod];
            ...
            NSDictionary *additionalAuthenticationParameters = [configuration
            ...
            if (additionalAuthenticationParameters != nil) {
                NSAssert([additionalAuthenticationParameters isKindOfClass:[NSDictionary class]],
                         @"additionalAuthenticationParameters have to be a NSDictionary");
...
- (NSString *)accountTypeOfPendingOAuthClient:(NXOAuth2Client *)oauthClient;
{
    ...
    @synchronized(self.pendingOAuthClients) {
        NSSet *accountTypes =
            [self.pendingOAuthClients keysOfEntriesPassingTest:^(id key, id obj, BOOL *stop) {
...
- (void)oauthClientDidGetAccessToken:(NXOAuth2Client *)client {
    ...
    NXOAuth2Account *account =
        [[NXOAuth2Account alloc] initAccountWithOAuthClient:client accountType:accountType];
...
- (void)addAccount:(NXOAuth2Account *)account;
{
    ...
    NSDictionary *userInfo =
        [NSDictionary dictionaryWithObject:account
                                    forKey:NXOAuth2AccountStoreNewAccountUserInfoKey];
...
- (void)oauthClient:(NXOAuth2Client *)client didFailToGetAccessTokenWithError:(NSError *)error;
{
    ...
    NSDictionary *userInfo =
        [NSDictionary dictionaryWithObjectsAndKeys:accountType, kNXOAuth2AccountStoreAccountType,
                                                   error, NXOAuth2AccountStoreErrorKey, nil];

    [[NSNotificationCenter defaultCenter]
        postNotificationName:NXOAuth2AccountStoreDidFailToRequestAccessNotification
                      object:self
                    userInfo:userInfo];
}
    ...
    trustedCertificatesForHostname:(NSString *)hostname;
{
    NSString *accountType = [self accountTypeOfPendingOAuthClient:connection.client];
    NXOAuth2TrustedCertificatesHandler handler =
        [self trustedCertificatesHandlerForAccountType:accountType];
...
- (void)accountDidLoseAccessToken:(NSNotification *)aNotification;
{
    NSLog(@"Removing account with id '%@' from account store because it lost its access token.",
...
- (NSDictionary *)accountsFromDefaultKeychainWithAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    NSMutableDictionary *query = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:(__bridge NSString *)kSecClassGenericPassword, kSecClass,
                                     serviceName, kSecAttrService, kCFBooleanTrue,
    ...
    if (status != noErr) {
        NSAssert1(status == errSecItemNotFound,
                  @"Unexpected error while fetching accounts from keychain: %zd", status);
        return nil;
    }

    return [NSKeyedUnarchiver
        unarchiveObjectWithData:[result objectForKey:(__bridge NSString *)kSecAttrGeneric]];
}
                       ...
                       withAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    NSMutableDictionary *query = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:(__bridge NSString *)kSecClassGenericPassword, kSecClass,
                                     serviceName, kSecAttrService, @"OAuth 2 Account Store",
                                     kSecAttrLabel, data, kSecAttrGeneric, nil];
...
- (void)removeFromDefaultKeychainWithAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    NSMutableDictionary *query = [NSMutableDictionary
        dictionaryWithObjectsAndKeys:(__bridge NSString *)kSecClassGenericPassword, kSecClass,
                                     serviceName, kSecAttrService, nil];
    ...
    NSAssert1((err == noErr || err == errSecItemNotFound),
              @"Error while deleting token from keychain: %zd", err);
}
...
- (NSDictionary *)accountsFromDefaultKeychainWithAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    OSStatus err =
        SecKeychainFindGenericPassword(NULL, strlen([serviceName UTF8String]),
                                       [serviceName UTF8String], 0, NULL, NULL, NULL, &item);
    if (err != noErr) {
        NSAssert1(err == errSecItemNotFound,
                  @"Unexpected error while fetching accounts from keychain: %d", err);
        return nil;
    }
                       ...
                       withAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    OSStatus __attribute__((unused)) err = SecKeychainAddGenericPassword(
        NULL, strlen([serviceName UTF8String]), [serviceName UTF8String], 0, NULL, [data length],
        [data bytes], NULL);
...
- (void)removeFromDefaultKeychainWithAccessGroup:(NSString *)keyChainAccessGroup;
{
    ...
    OSStatus err =
        SecKeychainFindGenericPassword(NULL, strlen([serviceName UTF8String]),
                                       [serviceName UTF8String], 0, NULL, NULL, NULL, &item);
    NSAssert1((err == noErr || err == errSecItemNotFound),
              @"Error while deleting accounts from keychain: %d", err);
    if (err == noErr) {
